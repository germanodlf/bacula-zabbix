...
case ${baculaDbSgdb} in
  P)
    ...
    sql="/usr/bin/psql -t -A -h$baculaDbAddr -p$baculaDbPort -U$baculaDbUser -d$baculaDbName -c"
    ;;
  M)
...
if [[ ${baculaClientNameReplacement} ]]; then baculaClientName=$(echo ${baculaClientName} | sed -r ${baculaClientNameReplacement}) ; fi
echo ${baculaClientName}
