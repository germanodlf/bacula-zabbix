...
source /etc/bareos/bacula-zabbix.conf
...
case $baculaDbSgdb in
  P) sql="/usr/bin/psql -h$baculaDbAddr -p$baculaDbPort -U$baculaDbUser -d$baculaDbName -tA -c" ;;
...
baculaJobName=$($sql "select name from Job where Job.JobId=$baculaJobId;")
if [ -z $baculaJobName ] ; then exit 15 ; fi
...
$zabbixSender -z $zabbixSrvAddr -p $zabbixSrvPort -s  $baculaClientName -k "bacula.$level.job.status[$baculaJobName]" -o $status >/dev/null 2>&1
...
$zabbixSender -z $zabbixSrvAddr -p $zabbixSrvPort -s  $baculaClientName -k "bacula.$level.job.bytes[$baculaJobName]" -o $baculaJobBytes >/dev/null 2>&1
...
$zabbixSender -z $zabbixSrvAddr -p $zabbixSrvPort -s $baculaClientName -k "bacula.$level.job.files[$baculaJobName]" -o $baculaJobFiles >/dev/null 2>&1
...
$zabbixSender -z $zabbixSrvAddr -p $zabbixSrvPort -s $baculaClientName -k "bacula.$level.job.time[$baculaJobName]" -o $baculaJobTime >/dev/null 2>&1
...
$zabbixSender -z $zabbixSrvAddr -p $zabbixSrvPort -s $baculaClientName -k "bacula.$level.job.speed[$baculaJobName]" -o $baculaJobSpeed >/dev/null 2>&1
...
$zabbixSender -z $zabbixSrvAddr -p $zabbixSrvPort -s $baculaClientName -k "bacula.$level.job.compr[$baculaJobName]" -o $baculaJobCompr >/dev/null 2>&1
